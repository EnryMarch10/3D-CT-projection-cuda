...
\begin{abstract}
La tomografia computerizzata (TC) è una tecnica diagnostica che sfrutta le radiazioni ionizzanti (o raggi X) per ottenere una
rappresentazione tridimensionale di un'entità analizzata.
Questa tecnica si basa sulla raccolta, da parte di un rilevatore, di più viste, dette proiezioni.
Tali proiezioni sono realizzate a partire da considerazioni fatte sull'attenuazione dei raggi X che, diretti da una sorgente verso
un rilevatore, passano attraverso l'oggetto di studio.
Facendo ruotare la sorgente ed il rilevatore, lungo una traiettoria nota, molte proiezioni vengono in genere raccolte.
Queste proiezioni vengono poi trasferite in un calcolatore che ha il compito di convertirle in immagini 2D e, se necessario,
di ricostruire l'entità scansionata in 3D.
La TC permette di analizzare l'oggetto ricostruito, evidenziando proprietà di densità della sua struttura interna precedentemente
ignote, senza la necessità di ricorrere a tecniche distruttive.

Nonostante molto sfruttata in ambito medico, la teoria su cui si basa la TC, in realtà, non fu ideata per scopi medici.
...
Il principio su cui si basa ha origine grazie al matematico austriaco Johann Radon, che dimostrò la possibilità di ricostruire un
oggetto tridimensionale mediante un numero infinito di proiezioni bidimensionali dell'oggetto stesso.
L'adattamento di queste teorie ad un numero finito di proiezioni ha reso possibile lo sviluppo di questa tecnica.
Dal punto di vista matematico, questo rappresenta un problema inverso, cioè si determinano le cause, oggetto 3D, a partire dagli
...
\begin{itemize}
  \item Implementare una versione per GPU NVIDIA ottimizzata dell'algoritmo di proiezione proposto da Siddon \cite{Siddon1984}
        partendo da una sua implementazione multicore per la CPU \cite{Colletta2024}.
  \item Analizzare le prestazioni della versione per GPU realizzata e confrontarle con la versione per CPU già nota.
\end{itemize}
\end{abstract}
...
La tomografia computerizzata a raggi X (TC) è una tecnica di imaging\footnote{Consente di esplorare le strutture interne di un
corpo in modo non invasivo.} introdotta nel 1972.
È ampiamente utilizzata per individuare eventuali anomalie nei volumi corporei esaminati.
In ambito medico per esempio è uno strumento essenziale per lo studio di molteplici condizioni cliniche.

La TC è frequentemente paragonata alla radiografia tradizionale.
Entrambe, la radiografia e la tomografia computerizzata, sono tecniche di imaging basate sui raggi X, ma hanno differenze
fondamentali in termini di acquisizione delle immagini, qualità e applicazioni.
La radiografia è una tecnica rapida ed economica che fornisce immagini bidimensionali, ma con sovrapposizione delle strutture e
di conseguenza minore dettaglio.
Espone l'oggetto ad una bassa dose di radiazioni ed è ideale per lo studio di strutture ad alto contrasto.
La tomografia computerizzata, invece, utilizza scansioni da diverse angolazioni per generare immagini dettagliate in sezioni
assiali, permettendo una migliore visualizzazione dell'oggetto.
I diversi risultati, cioè le proiezioni possono risultare utili in quanto tali o essere usate per una ricostruzione della
struttura dell'oggetto di studio in tre dimensioni.
Offre un contrasto superiore, ma ha un costo più elevato ed un'esposizione maggiore alle radiazioni rispetto alla radiografia
tradizionale.
...
La nascita dell'imaging medico risale a non molto tempo dopo che Wilhelm Röntge scoprì i raggi X, nel 1985.
...
sono state usate per decenni per investigare i volumi interni del corpo umano.
Comunque, queste immagini rappresentano solo una vista delle informazioni dell'oggetto 3D scannerizzato, rappresentato su un piano
2D.
Negli anni '30 del Novecento, la teoria matematica pubblicata da Johann Radon nel 1917, gli studi del fisico Grossmann, insieme al
desiderio di superare il processo della radiografia a raggi X, hanno portato alla definizione della tomografia come nuovo
strumento per ispezionare gli oggetti.
Dall'avvento dei computer, negli anni '70 del Novecento, la TC ha rivoluzionato l'imaging diagnostico, permettendo di ricostruire
...
Nell'ottobre del 1971, nel reparto radiologico dell'Atkinson Morley Hospital di Londra, fu prodotta la prima immagine di un
encefalo, dalla quale risultava chiaramente la presenza di una lesione tumorale nel lobo frontale.

Il prototipo, basato su movimenti di traslazione e rotazione del complesso tubo radiogeno-detettore, produceva un'immagine su una
matrice di \(80 \times 80\) con una risoluzione spaziale di 0,5 cm, e richiedeva per l'acquisizione e la ricostruzione di ciascuna
fetta, rispettivamente, 4 e 7 minuti.
...
\begin{figure}[h]
  ...
  \caption{\label{fig:history_mark_1} Qui rappresentato il primo tomografo computerizzato commerciale per lo studio del cranio
           prodotto dalla EMI nel 1973. Si tratta dell'EMI Mark 1 a sinistra, mentre a destra vi è il computer usato per
           processare i dati raccolti dal tomografo.}
\end{figure}

Nella \autoref{fig:history_mark_1} è rappresentato il primo tomografo computerizzato commerciale, denominato EMI Mark 1.

Oggi, grazie ai progressi tecnologici e all'innovazione nelle tecniche, la TC è sempre più diffusa e utilizzata.

\section{Tecniche}

Negli anni sono stati fatti vari studi per migliorare le tecnologie di scansione ed i metodi di ricostruzione software.
...
Osserva la \autoref{fig:history_tecnologies}.
La prima generazione di scanner per la TC, sviluppata negli anni '70, utilizzava un fascio di raggi X paralleli con movimenti
...
Successivamente, la seconda generazione introdusse l'uso di fasci a ventaglio, migliorando l'efficienza della scansione e
riducendo i tempi necessari per acquisire le immagini.
L'ulteriore evoluzione ha portato all'adozione di fasci a cono.
Questa tecnica utilizza un fascio di raggi X a forma di cono per acquisire immagini tridimensionali in una singola rotazione
attorno al paziente, offrendo dettagli ad alta risoluzione con una dose di radiazioni inferiore rispetto alle TC tradizionali.
...
Oltre alla geometria adottata, il numero di proiezioni acquisite influisce significativamente sulla qualità della ricostruzione
dell'oggetto, mentre, inversamente, incide sul tempo necessario per completarla.

\section{Processo}

Indipendentemente dalla tecnica, il processo di scansione nella TC consiste nell'acquisizione di numerose proiezioni dello stesso
oggetto in esame, da varie angolazioni diverse, seguendo una specifica traiettoria.
...
software per ricostruire un'immagine dettagliata della sua struttura interna.

Dal punto di vista matematico, si parla di:
\begin{itemize}
  \item Problema diretto: dall'oggetto alle proiezioni, consiste nell'acquisizioni del sinogramma, cioè l'insieme di tutte le
        proiezioni.
  \item Problema inverso: dalle proiezioni all'oggetto, consiste nella ricostruzione dell'oggetto.
\end{itemize}
...
vantaggi e limitazioni in termini di accuratezza, velocità di calcolo e robustezza ai disturbi.
...
Siddon \cite{Siddon1984}, utilizzato per simulare il passaggio dei raggi X attraverso l'oggetto ed ottenere infine il sinogramma.
...
Per confrontare i risultati e verificare la correttezza, si possono adottare due approcci:
\begin{itemize}
