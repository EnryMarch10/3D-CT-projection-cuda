...
\begin{abstract}
La tomografia computerizzata (TC) è una tecnica diagnostica molto utilizzata negli ultimi cinquant'anni che sfrutta radiazioni
ionizzanti (o raggi X) per ottenere una rappresentazione tridimensionale dell'entità in esame.
Questa tecnica si basa sulla raccolta, da parte di un rilevatore, di più viste, dette proiezioni.
Tali proiezioni sono date da considerazioni fatte sull'attenuazione dei raggi X che, diretti da una sorgente verso un rilevatore,
passano attraverso l'oggetto di studio.
Facendo ruotare la sorgente ed il rilevatore lungo una traiettoria nota, molte proiezioni vengono raccolte.
Le proiezioni ottenute vengono poi trasferite in un calcolatore che ha il compito di convertirle in immagini 2D che sono
essenziali per ricostruire il volume 3D dell'entità scansionata.
...
Nonostante sia molto sfruttata in ambito medico, la TC, in realtà, non fu ideata solamente per scopi medici ma trova applicazione
in vari settori industriali, la paleontologia, l'archeologia e numerosi altri campi.

La TC originò grazie agli studi del matematico austriaco Johann Radon, che dimostrò teoricamente la possibilità di ricostruire
un oggetto tridimensionale mediante un numero infinito di proiezioni bidimensionali dell'oggetto stesso.
L'adattamento di queste teorie all'uso di un numero finito di proiezioni ha reso possibile lo sviluppo di questa tecnica.
Dal punto di vista matematico, la ricostruzione dell'oggetto 3D a partire dalle proiezioni 2D rappresenta un problema inverso,
cioè si determinano le cause a partire dagli effetti osservati.
  ...
  \centering
    \begin{tikzpicture}[scale=0.8]
        \begin{axis}[
          ...
          title={Tempi di esecuzione OpenMP},
    ...
    \begin{tikzpicture}[scale=0.8]
        \begin{axis}[
          x=1cm,
          width=12cm, height=8cm,
          title={Tempi di esecuzione CUDA},
          xlabel={Configurazione},
          ylabel={Tempo di esecuzione (s)},
          xtick=data,
          xmin=0, xmax=13,
