...
\usepackage[figure]{hypcap} % Ensures that references go to the figure, not the caption
...
\subsection{Il vettore \(f\)} \label{chap:vector_f}
...
Le tre diverse configurazioni utilizzate sono:
\begin{itemize}
...
In particolare l'hardware delle GPU prevede esecuzione:
\begin{itemize}
  ...
  \item MIMD: a livello di multiprocessore
        \begin{itemize}
          \item Ogni Streaming Multiprocessor (SM) della GPU può eseguire più warps indipendenti, ognuno con istruzioni diverse.
...
Il meccanismo di computazione in unità, i Warp, implica che siano le computazioni SIMT che permettono le prestazioni migliori.
Questo perché le divergenze, istruzioni condizionali nel codice, portano alla serializzazione dell'esecuzione in Thread relativi
ad un singolo Warp.
...
Il \autoref{lst:vector_f_config} rappresenta in realtà un'iterazione della scrittura su file.
...
Tale partizione avrà numero di voxel sull'asse delle \(y\) pari a \lstinline{k}, dove \lstinline{k <= N_VOXELS_Y}.
Nella soluzione proposta oltretutto la singola iterazione presentata viene parallelizzata sfruttando OpenMP.

In questo modo, anche la lettura dell'input può essere fatta per partizioni sul numero totale di voxel nell'asse delle \(y\),
...
In particolare, nel caso di computazione su CPU, è fondamentale considerare la quantità di memoria RAM disponibile sulla scheda
madre (aspetto che approfondiremo con OpenMP).
Nel caso di computazione su GPU, invece, vanno prese in considerazione sia la memoria RAM della scheda madre sia quella
disponibile sulla GPU (aspetto che esamineremo con CUDA).
Ad esempio, supponiamo di dover analizzare una partizione di lunghezza \lstinline{OBJ_BUFFER}, con la condizione
\lstinline{OBJ_BUFFER <= N_VOXELS_Y}.
In questo caso, per quella partizione verranno letti solo \lstinline{N_VOXEL_X *  OBJ_BUFFER * N_VOXEL_Z} valori.

\section{Calcolo proiezioni}

L'algoritmo di calcolo delle proiezioni implementato segue quanto già discusso nell'introduzione:
\begin{enumerate}
...
In \autoref{fig:flowchart} presentato il diagramma di flusso relativo.
