...
for SOURCE in cuda-*.cu; do
  if [[ $SOURCE != *-test.cu ]]; then
    ...
    INPUT_MAX_DIM=1500 PROG_INPUT=./build/bin/inputgen DIR_INPUT=./inputs DIR_OUTPUT="./outputs/tput/$NAME" "$(dirname "$0")/demo-tput-cuda.sh" "./build/bin/$NAME" "$N_REPS" >> "$DIR_RESULTS/tput-$NAME.txt"
  fi
done
